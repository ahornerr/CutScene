...
function TestBuildDownloadFilename($$t->T) {$tests = []struct {
		name  string
		title string
		want  string
	}{
		...
		$t->Run($test->name, func(t *$testing->T) {$got = buildDownloadFilename(renderJobSpec{
...
function TestRenderDownloadContentDispositionSupportsUnicodeSafely($$t->T) {$header = renderDownloadContentDisposition(renderJobSpec{Title: "映画"})
